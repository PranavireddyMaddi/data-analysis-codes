...
iris = pd.read_csv("D:\\CQ\\Python\\datasets\\iris.csv")
...
np.mean(a)
# train accuracy 
train_acc = np.mean(neigh.predict(train.iloc[:,0:4])==train.iloc[:,4]) # 99.16%
...
final=KNC(n_neighbors=19)
final.fit(iris.iloc[:,0:4],iris.iloc[:,4])
np.mean(final.predict(iris.iloc[:,0:4])==iris["Species"])
